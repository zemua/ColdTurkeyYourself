...
class LockdownNeutralDecreaseViewBuilderTest {

    private LockdownNeutralDecreaseViewBuilder builder;
    ...
    @BeforeEach
    void setup() {
        builder = new LockdownNeutralDecreaseViewBuilder(preferencias, clickListenerFactoryProvider);
    }
    ...
    void testBuildElement() {
        ...
        when(preferencias.getBoolean(PreferencesEnum.LOCKDOWN_NEUTRAL_DECREASE, true)).thenReturn(true);

        builder.buildElement(parent, 123);

        verify(aSwitch, times(1)).setOnClickListener(clickListener);
        verify(preferencias, times(1)).getBoolean(PreferencesEnum.LOCKDOWN_NEUTRAL_DECREASE, true);
